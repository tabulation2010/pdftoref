...
$ x_9= $ \textit{Modified quadratic discriminant functions and the application to Chinese character recognition}\\
...
Come già citato nell'introduzione (sezione \ref{intro}) una volta separata ogni voce della bibliografia e ogni titolo con il metodo suddetto, si è implementato delle funzioni per cercare informazioni a riguardo dell'articolo citato identificato dal titolo. La soluzione scelta è stata quella di fare delle query al WebService di Google in maniera tale da realizzare una vera e propria ricerca al fine di reperire l'indirizzo che identifica l'articolo in rete. Ci cerca quindi nei primi tre risultati almeno un collegamento ad uno dei portali di maggiore rilevanza (~\htmladdnormallink{CiteSeer}{http://citeseer.ist.psu.edu/},~\htmladdnormallink{Portal.Acm}{http://portal.acm.org},~\htmladdnormallink{IEEExplore}{http://ieeexplore.ieee.org} ), per ottenere sia il Bibtex sia il PDF.\\
